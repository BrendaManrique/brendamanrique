...
async function compareResponses(v1Response: string, v2Response: string, testInput: string): Promise<{ better: 'v1' | 'v2' | 'equal'; reason: string }> {
  const result = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `Compare two chatbot responses to the same question. This is Brenda Manrique's portfolio AI — it speaks about her in the third person and must never overstate her record.
