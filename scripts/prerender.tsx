...
import { portfolioAgentContent } from '../src/articles/portfolio-agent-i18n.ts';
import { casicornioContent } from '../src/articles/casicornio-i18n.ts';
import { invipContent } from '../src/articles/invip-i18n.ts';
import { fractalContent } from '../src/articles/fractal-i18n.ts';
import { earlyProjectsContent } from '../src/articles/early-projects-i18n.ts';
...
const i18nMap: Record<string, Record<string, { header: { h1: string }; nav: { breadcrumbHome: string; breadcrumbCurrent: string }; faq: { items: readonly { q: string; a: string }[] } }>> = {
  'moodys': moodysContent,
  'financial-systems': financialSystemsContent,
  'consulting': consultingContent,
  'portfolio-agent': portfolioAgentContent,
  'casicornio': casicornioContent,
  'invip': invipContent,
  'fractal': fractalContent,
  'early-projects': earlyProjectsContent,
  'story': storyContent,
};
...
function buildArticlePage(
  ...
  let result = indexHtml
    ...
    .replace(/<meta property="og:image" content="[^"]*" \/>/, `<meta property="og:image" content="${esc(config.ogImage || '${SITE_URL}/og-image.webp')}" />`)
...
for (const [lang, slug, altSlug] of [['es', 'privacidad', 'privacy'], ['en', 'privacy', 'privacidad']] as const) {
  ...
  const altUrl = `${SITE_URL}/${altSlug}`;
  const altLang = lang === 'es' ? 'en' : 'es';
  const title = lang === 'es' ? 'Política de Privacidad | brendamanrique.com' : 'Privacy Policy | brendamanrique.com';
  const description = lang === 'es'
    ? 'Política de privacidad de brendamanrique.com. Cómo se recopilan y utilizan los datos del chat y la web.'
...
const notFoundHtml = indexHtml
  ...
  .replace(/<meta name="robots" content="[^"]*" \/>/, '<meta name="robots" content="noindex, nofollow" />')
  .replace(/<title>[^<]*<\/title>/, '<title>404 — Page not found | brendamanrique.com</title>');
