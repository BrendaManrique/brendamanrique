  ...
  useEffect(() => {
    ...
    if (phase === 'hook') {
      ...
      if (displayText === currentText) {
        ...
        // Beat after the first full stop of a hook line — lets the sentence land.
        const textSoFar = currentText.slice(0, nextCharIndex)
        const isAfterSentenceEnd = prevChar === '.' && nextChar === ' ' && textSoFar.indexOf('.') === nextCharIndex - 1
    ...
    <main className="min-h-screen bg-background bg-[length:24px_24px] [background-image:radial-gradient(circle,hsl(var(--dot-grid))_1px,transparent_1px)]">
      ...
      <footer id="contact" className="relative py-16 md:py-24">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <AnimatedSection>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-gradient-theme mb-4">
              {t.cta.title}
            </h2>
            ...
            <div className="pt-8 border-t border-border/50">
              <p className="text-xs text-muted-foreground leading-relaxed max-w-2xl mx-auto mb-3">
                {t.footerNote}
              </p>
              <div className="flex items-center justify-center gap-3 text-xs text-muted-foreground">
                <span>&copy; {new Date().getFullYear()} Brenda Manrique</span>
