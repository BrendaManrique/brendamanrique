...
import { AUTHOR_JOB_TITLE, AUTHOR_NAME, GITHUB_URL, LINKEDIN_URL, SITE_NAME, SITE_URL } from '../site'
...
interface JsonLdOptions {
  ...
  /** Publisher org — only when published with an external organization */
  ...
  const graph: Record<string, unknown>[] = [
    {
      ...
      // Override only when an article is published with an external organization.
    ...
    WEBSITE,
    {
      ...
      itemListElement: [
        { '@type': 'ListItem', '@id': `${opts.url}/#breadcrumb-1`, position: 1, name: opts.breadcrumbHome, item: SITE_URL },
...
export function buildJsonLdFromRegistry(
  ...
  return buildArticleJsonLd({
    lang,
    url: `${SITE_URL}/${i18n.slug}`,
    altUrl: `${SITE_URL}/${i18n.altSlug}`,
